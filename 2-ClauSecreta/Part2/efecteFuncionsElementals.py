...
class AES(object):
    ...
    def encryption(self, data, key):
        ...
        return int(self.aes_main(data, key, False), 16)
...
# Efecte de les funcions elementals
aes = AES(mode='ecb', input_type='hex')
...
def checkSubBytes():
    ...
    for i in range(128):
        ...
        for j in range(i+1,128):
            ...
            Mij = M ^ ((1 << i) | (1 << j))
            Cij = aes.encryption(Mij, K)    # ¿¿ si solo dos con K i dos con key??
            if (C != Ci ^ Cj ^ Cij): 
                return False    # no es lineal (bueno)
    return True     # es lineal (malo). Caso cuando ByteSub = ID

""" 
Al canviar SubBytes per la identitat, el xifrat es torna lineal, 
i podem comprobar amb checkSubBytes que per tot 'i','j',  C = Ci^Cj^C^ij
...
""" 
Al canviar ShiftRows per la identitat, quan xifrem un bloc M i un bloc Mi 
(on només un bit s'ha modificat respecte M i Mi) amb la mateixa clau K, 
el criptograma C i Ci són molt semblants, i només presenten diferencies
en una columna. 
...
""" 
Al canviar MixColumns per la identitat, els criptogrames C i Ci són
gairabé identics, on només s'ha modificat el únic byte que contenia 
el bit modificat. 
Exemple de XOR entre C i Ci:
    hex = 00000000000000000000000000740000
Veiem que tant MixColumns com ShiftRows són esencials
per la difusio en el xifrat
 """
